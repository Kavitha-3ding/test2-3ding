...
/** Map contact -> all common phone keys so backend schemas update correctly */
const aliasPhones = (obj = {}) => {
  const phoneVal = obj.contact ?? obj.mobile ?? obj.phone ?? obj.phoneNumber ?? "";
  return {
    ...obj,
    contact: phoneVal,
    mobile: phoneVal,
...
const OrderDetails = () => {
  ...
  useEffect(() => {
    ...
    socket.on("orderUpdated", ({ orderId: updatedOrderId, updatedOrder }) => {
      if (String(updatedOrderId) === String(orderId)) {
        ...
        initializeFileOptions(data);
        hydrateMetaFields(data);
        // keep the top bar email in sync when updates arrive via socket
        setEmail(data.email || data.shippingDetails?.email || data.billingDetails?.email || "");
      }
    });
  ...
  const fetchOrderDetails = async () => {
    try {
      ...
      setCouponDiscount(data.couponDiscount || 0);
      // prefer a saved form email if present; otherwise fall back
      setEmail(data.email || data.shippingDetails?.email || data.billingDetails?.email || "");
    } catch (e) {
      setError(e.message);
    } finally {
  ...
  const handleSubmitOrder = async () => {
    try {
      ...
      // ---- COMPAT: ensure phone fields match whatever the backend expects ----
      const billingPhones = aliasPhones(billingToSave);
      const shippingPhones = aliasPhones(shippingToSave);

      // COMPAT: address/pincode aliases (keep your previous behavior)
      const billingCompat = {
        ...billingPhones,
        billingAddress: billingPhones.address ?? billingPhones.billingAddress,
        pin: billingPhones.pincode ?? billingPhones.pin,
        pinCode: billingPhones.pincode ?? billingPhones.pinCode,
      };
      const shippingCompat = {
        ...shippingPhones,
        shippingAddress: shippingPhones.address ?? shippingPhones.shippingAddress,
        pin: shippingPhones.pincode ?? shippingPhones.pin,
        pinCode: shippingPhones.pincode ?? shippingPhones.pinCode,
      };

      // ---- Choose a primary/top-level email (prefer edited forms) + alias customerEmail ----
      const primaryEmail = (
        shippingCompat.email ||
        billingCompat.email ||
        email ||
        order.email ||
        ""
      ...
      const payload = {
        ...
        // editable meta
        email: primaryEmail,
        customerEmail: primaryEmail, // for backends that use customerEmail
        handledBy,
        printStatus,
        shippingStatus,
        trackingNumber,
      ...
      // keep the top bar email in sync with what the server accepted
      setEmail(data.email || data.shippingDetails?.email || data.billingDetails?.email || "");
