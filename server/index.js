...
const orderSchema = new mongoose.Schema({
  ...
  status: { type: String, default: "pending" }, // Add couponDiscount field
  createdAt: { type: Date, default: Date.now },
   // ⬇️ NEW meta + email
   email: { type: String, default: "" },
   handledBy: { type: String, default: "Support" },
   printStatus: {
     type: String,
     enum: ["Not Started", "Print in Progress", "Paused", "Completed"],
     default: "Not Started",
   },
   shippingStatus: {
     type: String,
     enum: ["Preparing for Ship", "Ready for Pickup", "Shipped", "Delivered", "Cancelled"],
     default: "Preparing for Ship",
   },
   trackingNumber: { type: String, default: "" },
   courierService: {
     type: String,
     enum: ["To be updated", "DTDC", "Blue Dart", "Delhivery", "India Post", "Self/Local"],
...
app.post("/submit-checkout", async (req, res) => {
  ...
  try {
    const updatedOrder = await Order.findOneAndUpdate(
      { orderId }, // use _id if you're using ObjectId
      {
        $set: {
          billingDetails: { name, email, mobile, billingAddress, city, pin, state },
          shippingAddress,
    ...
    res.status(200).json({ message: "Checkout submitted", order: updatedOrder });
  } catch (err) {
...
//         },
//       },
//       { new: true, upsert: true } // upsert ensures a new order is created if not found
//     );

//     io.emit('orderSubmitted', { orderId, order: updatedOrder });
...
app.post("/payment-confirmation", async (req, res) => {
  ...
  try {
    const updatedOrder = await Order.findOneAndUpdate(
      { orderId: orderId }, // match by your custom order ID
      {
        paymentStatus: true,
        razorpay_data: {
          razorpay_order_id,
...
app.post("/orders/feedback/:id", async (req, res) => {
  const orderId = req.params.id;
  const { rating, content, feedback } = req.body;

  try {
    const updatedOrder = await Order.findOneAndUpdate(
      { orderId: orderId }, // Match your orderId field
      {
        feedback: {
          rating,
          content,
    ...
    res.json({ message: "Feedback submitted", order: updatedOrder });
  } catch (error) {
...
app.post("/verify-payment", (req, res) => {
  ...
  console.log("🧾 Payment received for verification:");
  ...
  if (generated_signature === razorpay_signature) {
    ...
    Order.findOneAndUpdate({ orderId }, { status: "paid" }, { new: true })
      .then(() => res.json({ status: "success" }))
      .catch((err) => {
        console.error("Mongo update failed:", err);
        res.status(500).json({ status: "failure", reason: "DB update error" });
      });
  } else {
    res.status(400).json({ status: "failure", reason: "Signature mismatch" });
  }
});
...
app.post("/create-razorpay-order", async (req, res) => {
  ...
  try {
    const order = await Order.findOne({ orderId }); // ✅ Fix here

    if (!order) {
      return res.status(404).json({ error: "Order not found" });
    }

    const priceInPaise = Math.round(order.total * 100); // ✅ Fix here
...
app.put('/orders/:orderId', async (req, res) => {
  ...
  const {
    // meta + email coming from your frontend payload
    email,
    handledBy,
    printStatus,
    shippingStatus,
    ...
    // addresses (your UI sends these through)
    billingDetails,
    shippingDetails, // optional if you decide to support it
  } = req.body;
    ...
    const updatedOrder = await Order.findOneAndUpdate(
      { orderId },
      {
        $set: {
          ...
          files: (files || []).map(f => ({
            ...f,
            itemTotal: f.itemTotal || 0,
            customPrice: f.customPrice ?? 0,
          })),
          ...
          shippingDetails: shippingDetails || undefined, // ignore if you don’t send it
          updatedAt: new Date(),
        },
      },
