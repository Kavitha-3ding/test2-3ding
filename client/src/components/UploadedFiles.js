...
//     qualityCosts: {}
//   });
//   const [appliedCoupon, setAppliedCoupon] = useState(null); // Store the applied coupon
...
//           buildVolume: fileToAccept.buildVolume || 1, // Default build volume if missing
...
//     const fileSubtotal = calculateItemTotal(
//       fileOptions[file._id]?.material || 'PLA',
//       fileOptions[file._id]?.density || '20%',
//       fileOptions[file._id]?.quality || 'Draft',
//       file.buildVolume,
...
//     // Apply discount only if the file's technology matches the coupon's technology
//     const isMatchingTechnology = appliedCoupon && fileOptions[file._id]?.technology === appliedCoupon.technology;
//     const discountedSubtotal = isMatchingTechnology
//       ? fileSubtotal - (fileSubtotal * couponDiscount) / 100
//       : fileSubtotal;

//     return acc + discountedSubtotal;
//   }, 0) + gst + shippingCharges;
...
//     if (!coupon || !coupon.technology) {
//       alert('Invalid coupon or technology not specified.');
//       return;
//     }

//     // Filter files that match the coupon's technology
//     const matchingFiles = files.filter(
//       (file) => fileOptions[file._id]?.technology === coupon.technology
//     );

//     if (matchingFiles.length === 0) {
//       alert(`No files match the technology "${coupon.technology}" for this coupon.`);
//       return;
//     }

//     // Apply the discount only to matching files
//     const discount = coupon.discount || 0;
//     setCouponDiscount(discount); // Update the discount in the state
//     setAppliedCoupon(coupon); // Store the applied coupon
...
//             {/* <tbody>
//               {files.map((file, index) => (
//                 <tr key={file._id || index} className="justify-content-center text-center">
//                   <td className="py-2 text-center">{index + 1}</td>
...
//                       fileOptions[file._id]?.density || '20%',
//                       fileOptions[file._id]?.quality || 'Draft',
//                       file.buildVolume || 1 // Default build volume if missing
//                     )}
//                   </td>
//                   <td>
//                     <input
          ...
          <button
            ...
            onClick={() => {
              if (!orderId || orderId === "null") {
                console.error("Invalid orderId. Cannot navigate to checkout.");
                alert("Order ID is not valid. Please try again.");
                return;
              }
              window.location.href = `/checkout/${orderId}`;
            }}
          >
            {files.length === 0 ? 'Checkout' : 'Checkout'}
          </button>
